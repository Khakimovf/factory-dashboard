                    ...
                    <aside className="hidden xl:flex w-72 shrink-0 flex-col gap-5 p-6 sticky top-[105px] self-start h-[calc(100vh-105px)] overflow-y-auto border-l border-slate-800 bg-slate-900/30 backdrop-blur-xl [&::-webkit-scrollbar]:hidden">
                        ...
                        <div className="bg-slate-900/80 rounded-2xl border border-slate-800 p-5 shadow-xl">
                            ...
                            <div className="relative h-3 bg-slate-900 rounded-full overflow-hidden border border-slate-800 shadow-inner">
                                <div className={`absolute top-0 left-0 h-full rounded-full transition-all duration-1000 ${excess ? 'bg-orange-500 shadow-[0_0_10px_rgba(249,115,22,0.6)]' : 'bg-cyan-500 shadow-[0_0_10px_rgba(6,182,212,0.5)]'}`} style={{ width: `${shiftPct}%` }} />
                            </div>
                            ...
                            {excess && <div className="flex items-center gap-1.5 mt-2 text-[10px] text-orange-400 bg-orange-500/10 px-3 py-2 rounded-lg border border-orange-500/20"><AlertTriangle className="w-3 h-3 shrink-0" />+{requiredMin - SHIFT_CAP} min over</div>}
                        </div>
